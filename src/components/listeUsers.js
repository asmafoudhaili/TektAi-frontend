...
useEffect(() => {
  const fetchChallengers = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get("http://localhost:9091/user/getallchallenger", {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      setChallengers(response.data.users); // Assurez-vous de récupérer la propriété 'users' de la réponse
    } catch (error) {
      console.error('Erreur lors de la récupération des challengers:', error);
    }
  };
...
useEffect(() => {
  const fetchCompanies = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get("http://localhost:9091/user/getallcompany", {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      setCompanies(response.data.users); // Assurez-vous de récupérer la propriété 'users' de la réponse
    } catch (error) {
...
const filteredChallengers = challengers.filter(user => {
  const nameMatches = user.firstname.toLowerCase().includes(searchValue.toLowerCase());
  if (filter === 'blocked') {
    return user.block && nameMatches;
...
const filteredCompanies = companies.filter(user => {
  const nameMatches = user.firstname.toLowerCase().includes(searchValue1.toLowerCase());
  if (filter1 === 'blocked') {
    return user.block && nameMatches;
  } else if (filter1 === 'notBlocked') {
    return !user.block && nameMatches;
  } else {
    return nameMatches;
  }
...
const handlePageClick1 = (direction) => {
  if (direction === 'prev') {
...
const challengerPerPage = 3;

const offset = currentPage * challengerPerPage;
const pageCount = Math.ceil(filteredChallengers.length / challengerPerPage);

const companyPerPage = 3;

const offset1 = currentPage1 * companyPerPage;
const pageCount1 = Math.ceil(filteredCompanies.length / companyPerPage);
  ...
  <div className="form-check form-check-inline mt-2" style={{ marginRight: '1rem' }}>
    <input
      type="radio"
  ...
  <div className="input-group mb-3" >
    ...
    <div class="col-6">
        <h5 class="card-title mb-0">Challengers</h5>
    </div>
...
<div className="table-responsive">
    <Table className="table table-hover my-0" style={{ borderCollapse: 'collapse', width: '100%', marginTop: '20px' }}>
        ...
        <Tbody>
            {filteredChallengers.slice(offset, offset + challengerPerPage).map((user, index) => (
             ...
             <div className="nk-pagination-wrap d-flex flex-wrap flex-sm-nowrap align-items-center gap g-3 justify-content-center justify-content-md-between pt-5 pt-lg-7 ">
                        <div className="nk-pagination-col">
                          <p>Showing: <span>{offset + 1}-{Math.min(offset + challengerPerPage, filteredChallengers.length)} of {filteredChallengers.length} </span></p>
                        </div>
                        <div className="nk-pagination-col">
        ...
        <hr style={{ margin: '30px auto', borderTop: '1px solid #ccc' ,width: '90%'}} />
  ...
  <div style={{ display: 'flex' }}>
    ...
    <div className="form-check form-check-inline mt-2" style={{ marginLeft: '0rem' }}>
      <input
        ...
        id="blocked"
        value="blocked"
        checked={filter1 === 'blocked'}
        onChange={() => setFilter1('blocked')}
        style={{ width: '13px', height: '13px', marginTop: '4px' }}
    ...
    <div className="form-check form-check-inline mt-2" style={{ marginLeft: '0rem' }}>
      <input
        ...
        id="notBlocked"
        value="notBlocked"
        checked={filter1 === 'notBlocked'}
        onChange={() => setFilter1('notBlocked')}
        style={{ width: '13px', height: '13px', marginTop: '4px' }}
...
<div className="table-responsive">
    <Table className="table table-hover my-0" style={{ borderCollapse: 'collapse', width: '100%', marginTop: '20px' }}>
        ...
        <Tbody>
            {filteredCompanies.slice(offset1, offset1 + companyPerPage).map((user, index) => (
                <Tr key={user._id} className="table-row" style={{ borderBottom: '1px solid #dee2e6', padding: '10px 0' }}>
                    <Td>
                        <img src='/images/no_pdp.jpg' style={{ width: '40px', height: '35px', borderRadius: '50%' }} />
                    </Td>
                            ...
                            <>
                                <Th>
                                    <FontAwesomeIcon
                                        ...
                                        style={{ cursor: 'pointer', marginRight: '5px', color: 'rgb(54, 162, 235)' }}
                                        size="lg"
                                        onClick={() => handleOpenUpdateForm1(user)}
                                    />
                                    <FontAwesomeIcon
                                        icon={faTrash}
                                        style={{ cursor: 'pointer', marginRight: '5px', color: 'rgb(255, 99, 132)' }}
             ...
             <div className="nk-pagination-wrap d-flex flex-wrap flex-sm-nowrap align-items-center gap g-3 justify-content-center justify-content-md-between pt-5 pt-lg-7 ">
                        <div className="nk-pagination-col">
                          <p>Showing: <span>{offset1 + 1}-{Math.min(offset1 + companyPerPage, filteredCompanies.length)} of {filteredCompanies.length} </span></p>
                        </div>
                        <div className="nk-pagination-col">
                          <nav aria-label="Page navigation example">
                            <ul className="pagination pagination-s1">
                              <li className={`page-item ${currentPage1 === 0 ? 'disabled' : ''}`}>
                                <button className="page-link" onClick={() => handlePageClick1('prev')}>
                                  <div name="chevron-left" />
                                  <span className="d-none d-sm-inline-block">Prev</span>
                                </button>
                              </li>
                              <li className="page-item active">
                                <span className="page-link">{currentPage1 + 1}</span>
                              </li>
                              <li className={`page-item ${currentPage1 === pageCount1 - 1 ? 'disabled' : ''}`}>
                                <button className="page-link" onClick={() => handlePageClick1('next')}>
                                  <span className="d-none d-sm-inline-block">Next</span>
                                  <div name="chevron-right" />
                                </button>
                              </li>
      ...
      <Modal.Body>
        ...
        <Row>
          ...
          <div className='text-center mt-5'>
            ...
            <Button variant="secondary" onClick={handleCloseModal4} size="sm">
              Cancel
            </Button>
          </div>
        </Form>
