...
const PageFourCompany =  ({ formData, setFormData, onButtonClick }) => {
  ...
  const handleSubmit = async () => {
    ...
    localStorage.setItem('challengerId', challengerIdFromResponse);

       console.log(  'Response from server:',    companyData.email);
       console.log( response.formData);
 
       if (response.status === 200) {
         // Save the formformData in local storage
...
export default PageFourCompany;